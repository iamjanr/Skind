...
+
When using EKS with managed node groups (_MachinePools_), the cluster-autoscaler uses static credentials from the provisioning user (`cloud-provisioner-eks` or equivalent). This user must have the following permissions in their IAM policy to allow the cluster-autoscaler to manage Auto Scaling Groups directly:
+
[source,json]
----
{
...
+
Additionally, the `eks-nodegroup.cluster-api-provider-aws.sigs.k8s.io` IAM role used by EKS managed node groups must have these same permissions attached. Create a customer-managed IAM policy (for example, named `keos-cluster-autoscaler`) with the actions above and attach it manually to the `eks-nodegroup.cluster-api-provider-aws.sigs.k8s.io` role after the CloudFormation stack is created.
